...
- ✅ Using `wolfpack_members_unified` table (not view)
...
CREATE INDEX CONCURRENTLY idx_wolfpack_members_unified_location_status 
ON wolfpack_members_unified(location_id, status) WHERE status = 'active';
...
AND table_name IN ('wolfpack_members_unified', 'wolf_chat', 'dj_events');
```
