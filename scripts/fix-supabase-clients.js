...
function analyzeSupabaseQueries() {
  ...
  for (const filePath of allFiles) {
    ...
    if (content.includes('wolfpack_members_unified')) {
      const lines = content.split('\n');
      lines.forEach((line, index) => {
        if (line.includes('wolfpack_members_unified')) {
          apiCalls.push({
            file: relativePath,
            line: index + 1,
            code: line.trim(),
            type: 'wolfpack_members_unified query'
          });
        }
      });
    }
  ...
  console.log('🎯 WOLFPACK_MEMBERS_UNIFIED QUERIES:');
  console.log('====================================');
  if (apiCalls.length === 0) {
    console.log('✅ No direct wolfpack_members_unified queries found\n');
  } else {
  ...
  const failingPattern = 'wolfpack_members_unified?select=id%2Clocation_id%2Cjoined_at%2Cstatus%2Cis_active&user_id=eq.';
  ...
  console.log('2. Verify RLS policies on wolfpack_members_unified table');
